...
// initialize firebase
initializeApp(
  Updates.releaseChannel == "stag" ? firebaseConfigStaging : firebaseConfigProd
);
...
export function partiesListener(uid, callback) {
  return onSnapshot(
    query(collection(firestore, "parties"), where("user_" + uid, "==", true)),
    callback
  );
}
...
export async function usernameExists(username) {
  ...
  if (
    unDocs.length == 0 ||
    (unDocs.length == 1 && unDocs[0].id == auth.currentUser.uid)
  )
    return false;
  return unDocs[0];
}
...
export function updateUserData(userData, data) {
  if (userData)
    return updateDoc(doc(firestore, "users", auth.currentUser.uid), data);
...
export function acceptRequest(uid) {
  return new Promise(async (resolve, reject) => {
    const modFriend = {
      ...
      outgoingRequests: arrayRemove(auth.currentUser.uid),
      score: increment(1),
    };
    await updateDoc(doc(firestore, "users", uid), modFriend);
...
export function getAnnouncements() {
  return getDocs(
    query(collection(firestore, "announcements"), where("visible", "==", true))
  );
}
